...
public class NumberOfBoomerangs {
    public int numberOfBoomerangs(int[][] points) {
        ...
        for (int i = 0; i < points.length; i++) {
            ...
            for (Integer nodes : nodeDistances.keySet()) {
                int nodeCount = nodeDistances.get(nodes);
                if(nodeCount>1){
                    count += nodeCount * (nodeCount -1);
                }
            }
            nodeDistances.clear();
        }
    ...
    public static void main(String[] args) {
        ...
        int i = new NumberOfBoomerangs().numberOfBoomerangs(points2);
